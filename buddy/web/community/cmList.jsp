...
<%
    ...
    while (rs.next()) {
        ...
        cm.setResdate(rs.getString("resdate"));
        cm.setCnt(rs.getInt("cnt"));
        cm.setLev(rs.getInt("lev"));
        cm.setPar(rs.getInt("par"));
        cm.setName(rs.getString("name"));
        cmList.add(cm);
    }

    sql = "SELECT community.comuno AS comuno, COUNT(commucomment.cno) as countCmt FROM community LEFT JOIN commucomment ON community.comuno = commucomment.comuno GROUP BY community.comuno, community.title";
    pstmt = conn.prepareStatement(sql);
    rs = pstmt.executeQuery();
    Map<Integer, Integer> cmtMap = new HashMap<>();
    while (rs.next()) {
        cmtMap.put(rs.getInt("comuno"), rs.getInt("countCmt"));
    }
                ...
                <div class="page_wrap">
                    ...
                    <table class="tb1" id="myTable">
                        ...
                        <%
                            ...
                            int tot = cmList.size();
                            for (Community q : cmList) {
                                Date d = ymd.parse(q.getResdate());
                                String date = ymd.format(d);
                                int cmtNum = cmtMap.get(q.getComuno());
                        %>
                        <tr>
                            <td class="item1"><%=tot %>
                            </td>
