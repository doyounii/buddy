...
<%
    ...
    //5. 실행결과(ResultSet) 인 해당 com 1건 com(질문및답변) 객체에 넣기
    ...
    //pstmt.close();
    sql = "update community set cnt=cnt+1 where comuno=?";
    pstmt = conn.prepareStatement(sql);
    pstmt.setInt(1, comuno);
    pstmt.executeUpdate();

    sql = "select * from commucomment where comuno=? order by cno";
    pstmt = conn.prepareStatement(sql);
    pstmt.setInt(1, comuno);
    rs = pstmt.executeQuery();
    List<CommuComment> cmtList = new ArrayList<>();
    while (rs.next()) {
        CommuComment cmt = new CommuComment();
        cmt.setCno(rs.getInt("cno"));
        cmt.setComuno(rs.getInt("comuno"));
...
<head>
    ...
    background-color: #eb9362;
        ...
        .comment-form { margin: 5px 160px;
            border: 1px solid #ccc; padding: 10px; border-radius: 5px; width: 76%;
        }
        ...
        .comment-form button {
            margin-top: 10px; padding: 8px 16px; background-color: #eb9362;
        ...
        .comment-table th { background-color: #eb9362; color: #fff }

        .comment-info { margin-bottom: 10px; }
        .comment-date { margin-left: 20px; font-size: 13px;  color: #777; }
        .comment-write {
            margin-top: 7px; padding: 5px; background-color: #eb9362; color: #fff;
            border: none;
        }
...
<div class="container">
    <div class="wrap">
        ...
        <div class="contents" id="contents">
            <div class="breadcrumb">
                <p><a href="/">HOME</a> &gt; <span>커뮤니티</span> &gt; <span>글 상세보기</span></p>
            </div>
            <section class="page" id="page1">
                <div class="page_wrap">
                    ...
                    <table class="tb1" id="myTable">
                        ...
                        <tr>
                            <td colspan="2">
                                ...
                                <% if(com.getLev()==0) { %>
                                <% if(sid!=null) { %>
                                <a href="/community/addCm.jsp?lev=1&par=<%=com.getComuno() %>" class="inbtn">답변하기</a>
                                <% } %>
                                <% if(sid!=null && (sid.equals("admin") || sid.equals(com.getAuthor()))) { %>
                                <a href="/community/updateCm.jsp?comuno=<%=com.getComuno() %>" class="inbtn">게시글 수정하기</a>
                                <a href="/community/delCm.jsp?comuno=<%=com.getComuno() %>&lev=0" class="inbtn">게시글 삭제하기</a>
                                <% } %>
                                <% } else { %>
                                <% if(sid!=null && (sid.equals("admin") || sid.equals(com.getAuthor()))) { %>
                                <a href="/community/updateCm.jsp?comuno=<%=com.getComuno() %>" class="inbtn">답변 수정하기</a>
                                <a href="/community/delCm.jsp?comuno=<%=com.getComuno() %>&lev=1" class="inbtn">답변 삭제하기</a>
                                <% } %>
                                <% } %>
                        ...
                        <tr>
                            <td>
                                <div class="comment-info">
                                    ...
                                    <a href="javascript:delComment('<%=c.getComuno() %>', '<%=c.getCno() %>')" class="del">삭제</a>
                                    <% } %>
                                </div>
                    ...
                    %>
                    <form action="/community/addCommentPro.jsp" method="post" class="comment-form">
                        <div class="comment-input">
                            ...
                            %>
                            <input type="hidden" name="comuno" id="comuno" value="<%=comuno %>">
                        </div>
    ...
    <script>
        function delComment(comuno, cno) {
            var flag = confirm("댓글을 삭제하시겠습니까?");
            if (flag) {
                location.href = "/community/delCommentPro.jsp?comuno=" + comuno + "&cno=" + cno;
            }
        }
