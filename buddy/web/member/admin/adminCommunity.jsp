...
<%@ page import="com.buddy.vo.Community" %>
...
<%
    ...
    String sql = "select comuno, title from community order by comuno desc";
    pstmt = conn.prepareStatement(sql);
    rs = pstmt.executeQuery();

    List<Community> boardList = new ArrayList<>();
    while (rs.next()) {
        Community bd = new Community();
        bd.setComuno(rs.getInt("comuno"));
    ...
    <div class="wrap">
        ...
        <div class="contents" id="contents">
            <div class="breadcrumb">
                <p><a href="">HOME</a> &gt; <span>관리자 페이지</span> &gt; <span>커뮤니티 관리</span></p>
            </div>
            <section class="page" id="page1">
                <div class="page_wrap">
                    <h2 class="page_tit">Buddy 라운지 관리</h2>
                    <hr>
                    <div class="member">
                        <table class="tb1" id="myTable">
                            ...
                            <tbody>
                            <%
                                for (Community b : boardList) {
                            %>
                            <tr>
                                <td class="item1"><%=b.getComuno() %></td>
                                <td class="item2"><%=b.getTitle()%></td>
                                <td class="item3 kick"><button class="inbtn" onclick="administerBoard('<%=b.getComuno() %>', 0)">수정</button></td>
                                <td class="item4 kick"><button class="inbtn" onclick="administerBoard('<%=b.getComuno() %>', 1)">삭제</button></td>
                            </tr>
                            <%
                                }
                            %>
                        ...
                        <script>
                            function administerBoard(bno, mode) {
                                if (mode == 0) {
                                    window.location.href = "/community/updateCm.jsp?comuno="+bno+"&mode=0";
                                } else {
                                    window.location.href = "/community/delCm.jsp?bno="+comuno+"&mode=1";
                                }
                            }
                        </script>
                    </div>
